...
namespace Stealer
{
    public class Spy
    {
        ...
        public string CollectGettersAndSetters(string className)
        {
            ...
            MethodInfo[] AllGettersAndSetters = classType.GetMethods
                (BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            
            StringBuilder sb = new StringBuilder();

            foreach (MethodInfo method in AllGettersAndSetters.Where(m=> m.Name.StartsWith("get")))
            {
                sb.AppendLine($"{method.Name} will return {method.ReturnType}");
            }

            foreach (MethodInfo method in AllGettersAndSetters.Where(m => m.Name.StartsWith("set")))
            {
